...
export function createAnalyticsRouter(pool: Pool): Router {
  ...
  const analyticsService = new AnalyticsService(pool, subscriptionService, emailService);
  ...
  const parseTimeframe = (req: Request) => {
    const { period = '30d', comparison } = req.query;

    const validPeriods = ['7d', '30d', '90d', '1y', 'all'];
    const validPeriod = validPeriods.includes(period as string) ? period as any : '30d';

    const timeframe: any = { period: validPeriod };
    if (comparison && validPeriods.includes(comparison as string)) {
      timeframe.comparison = comparison as any;
    }
  ...
  router.get('/traffic', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
  ...
  router.get('/engagement', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
  ...
  router.post('/track/social-share', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/track/newsletter-signup', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/revenue/ad', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
      ...
      await analyticsService.recordAdRevenue(
        req.user.id,
        source,
        revenue,
        impressions,
  ...
  router.get('/summary', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const timeframe = { period: `${days}d` as any };
