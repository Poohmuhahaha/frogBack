...
export class AffiliateLinkStats {
  private pool: Pool;
  ...
  async create(statsData: CreateAffiliateLinkStatsData): Promise<AffiliateLinkStatsData> {
    const id = uuidv4();
  ...
  async findById(id: string): Promise<AffiliateLinkStatsData | null> {
    const query = 'SELECT * FROM affiliate_link_stats WHERE id = $1';
    const result = await this.pool.query(query, [id]);
    return result.rows[0] || null;
  }

  async findByLinkId(linkId: string): Promise<AffiliateLinkStatsData[]> {
    const query = `
  ...
  async findByArticleId(articleId: string): Promise<AffiliateLinkStatsData[]> {
    const query = `
      SELECT * FROM affiliate_link_stats
      WHERE article_id = $1
      ORDER BY clicked_at DESC
  ...
  async findMany(filters: AffiliateLinkStatsFilters = {}): Promise<{ stats: AffiliateLinkStatsData[]; total: number }> {
  ...
  async update(id: string, updateData: UpdateAffiliateLinkStatsData): Promise<AffiliateLinkStatsData | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;
  ...
  async markConverted(id: string, commissionAmount: number): Promise<AffiliateLinkStatsData | null> {
  ...
  async getAnalytics(linkId: string, days: number = 30): Promise<ClickAnalytics> {
  ...
  async getTimeSeriesData(linkId: string, days: number = 30): Promise<TimeSeriesData[]> {
    const query = `
      SELECT
  ...
  async getTopPerformingArticles(linkId: string, limit: number = 10, days: number = 30): Promise<Array<{
    article_id: string;
    article_title: string;
    clicks: number;
    conversions: number;
  ...
  async getDailyCommission(creatorId: string, date: Date): Promise<number> {
    const query = `
  ...
  async getMonthlyCommission(creatorId: string, year: number, month: number): Promise<number> {
    const query = `
  ...
  async trackClick(linkId: string, articleId?: string, ipAddress?: string, userAgent?: string, referrer?: string): Promise<AffiliateLinkStatsData> {
    return this.create({
      link_id: linkId,
  ...
  async bulkCreate(statsDataArray: CreateAffiliateLinkStatsData[]): Promise<AffiliateLinkStatsData[]> {
  ...
  async deleteOldStats(daysToKeep: number = 365): Promise<number> {
    const query = `
