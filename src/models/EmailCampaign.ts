...
export class EmailCampaign {
  ...
  async create(campaignData: CreateEmailCampaignData): Promise<EmailCampaignData> {
    const id = uuidv4();
    const now = new Date();
  ...
  async findById(id: string): Promise<EmailCampaignData | null> {
    const query = 'SELECT * FROM email_campaigns WHERE id = $1';
    const result = await this.pool.query(query, [id]);
    return result.rows[0] || null;
  }

  async findByCreatorId(creatorId: string): Promise<EmailCampaignData[]> {
    const query = `
  ...
  async findMany(filters: EmailCampaignFilters = {}): Promise<{ campaigns: EmailCampaignData[]; total: number }> {
  ...
  async findScheduled(): Promise<EmailCampaignData[]> {
    const query = `
      SELECT * FROM email_campaigns
  ...
  async update(id: string, updateData: UpdateEmailCampaignData): Promise<EmailCampaignData | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;
  ...
  async schedule(id: string, scheduledAt: Date): Promise<EmailCampaignData | null> {
  ...
  async startSending(id: string, recipientCount: number): Promise<EmailCampaignData | null> {
    return this.update(id, {
      status: 'sending',
  ...
  async markSent(id: string): Promise<EmailCampaignData | null> {
    return this.update(id, {
      status: 'sent',
      sent_at: new Date()
    });
  ...
  async markFailed(id: string): Promise<EmailCampaignData | null> {
    return this.update(id, {
      status: 'failed'
    });
  }

  async updateStats(id: string): Promise<EmailCampaignData | null> {
  ...
  async getStats(creatorId: string): Promise<EmailCampaignStats> {
    const query = `
      SELECT
  ...
  async getRecentCampaigns(creatorId: string, limit: number = 5): Promise<EmailCampaignData[]> {
    const query = `
      SELECT * FROM email_campaigns
  ...
  async duplicate(id: string, newName: string): Promise<EmailCampaignData | null> {
    const original = await this.findById(id);
    if (!original) return null;

    const duplicateData: CreateEmailCampaignData = {
