...
exports.createAffiliatesRouter = createAffiliatesRouter;
const express_1 = require("express");
const AffiliateService_1 = require("../services/AffiliateService");
function createAffiliatesRouter(pool) {
    const router = (0, express_1.Router)();
    const affiliateService = new AffiliateService_1.AffiliateService(pool);
    ...
    router.get('/links', authenticateToken, async (req, res) => {
        try {
            ...
            const { network, category, is_active, search, limit = 20, offset = 0 } = req.query;
            const filters = {
    ...
    router.post('/links', authenticateToken, async (req, res) => {
        try {
            ...
            const { name, originalUrl, network, commissionRate, category } = req.body;
    ...
    router.put('/links/:id', authenticateToken, async (req, res) => {
        try {
            ...
            const { id } = req.params;
            const { name, original_url, commission_rate, category, is_active } = req.body;
            const updateData = {};
            if (name !== undefined)
                updateData.name = name.trim();
            if (original_url !== undefined)
                updateData.original_url = original_url.trim();
    ...
    router.post('/links/:id/activate', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
    ...
    router.post('/links/:id/deactivate', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
    ...
    router.post('/links/:id/regenerate-code', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
    ...
    router.post('/conversions', async (req, res) => {
        try {
            const { trackingCode, orderId, commissionAmount, conversionDate, metadata } = req.body;
    ...
    router.get('/analytics/:id', authenticateToken, async (req, res) => {
        try {
            ...
            const analytics = await affiliateService.getLinkAnalytics(id, req.user.id, parseInt(days) || 30);
            res.json({
                analytics
            });
        }
    ...
    router.get('/summary', authenticateToken, async (req, res) => {
        try {
            ...
            const { days = 30 } = req.query;
            const summary = await affiliateService.getCreatorSummary(req.user.id, parseInt(days) || 30);
            res.json({
                summary
            });
        }
    ...
    router.get('/analytics/network-performance', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
            const { days = 30 } = req.query;
            const networkPerformance = await affiliateService.getNetworkPerformance(req.user.id, parseInt(days) || 30);
            res.json({
                networkPerformance
            });
        }
    ...
    router.get('/analytics/monthly-commission', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
            const { months = 12 } = req.query;
            const monthlyCommission = await affiliateService.getMonthlyCommissionBreakdown(req.user.id, parseInt(months) || 12);
            res.json({
                monthlyCommission
            });
        }
    ...
    router.get('/links/:id/clicks', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
            ...
            const clickHistory = await affiliateService.getClickHistory(id, req.user.id, parseInt(limit) || 50);
            res.json({
                clicks: clickHistory
            });
        }
    ...
    router.get('/export', authenticateToken, async (req, res) => {
        try {
            ...
            const exportData = await affiliateService.exportAnalytics(req.user.id, format, parseInt(days) || 30);
            if (format === 'csv') {
    ...
    router.get('/url/:id', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
            ...
            const trackedUrl = await affiliateService.buildTrackedUrl(id, baseUrl);
            if (!trackedUrl) {
                return res.status(404).json({ error: 'Affiliate link not found' });
            }
            res.json({
