...
function createAuthRouter(pool) {
    ...
    const authService = new AuthService_1.AuthService(pool);
    ...
    const authenticateToken = async (req, res, next) => {
        try {
            ...
            if (!AuthService_1.AuthService.isValidTokenFormat(token)) {
    ...
    router.post('/register', async (req, res) => {
        try {
            ...
            if (!User_1.User.validateEmail(email)) {
                return res.status(400).json({
                    error: 'Invalid email format'
                });
            }
            // Validate password strength
            const passwordValidation = User_1.User.validatePassword(password);
            ...
            // Validate role
            if (!User_1.User.validateRole(role)) {
                return res.status(400).json({
    ...
    router.put('/profile', authenticateToken, async (req, res) => {
        try {
            ...
            const user = new User_1.User(pool);
            const updateData = {};
            if (name !== undefined)
