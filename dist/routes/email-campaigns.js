...
exports.createEmailCampaignsRouter = createEmailCampaignsRouter;
const express_1 = require("express");
const EmailService_1 = require("../services/EmailService");
function createEmailCampaignsRouter(pool) {
    ...
    router.post('/', authenticateToken, async (req, res) => {
        try {
            ...
            const { name, subject, content, type = 'newsletter', scheduled_at } = req.body;
    ...
    router.put('/:id', authenticateToken, async (req, res) => {
        try {
            ...
            const { id } = req.params;
            const { name, subject, content, type, scheduled_at } = req.body;
            const updateData = {};
            if (name !== undefined)
    ...
    router.post('/:id/send', authenticateToken, async (req, res) => {
        try {
            ...
            const { id } = req.params;
            const { segment_filters = {}, exclude_unsubscribed = true, send_at } = req.body;
            const sendRequest = {
                campaign_id: id,
                segment_filters,
    ...
    router.post('/:id/duplicate', authenticateToken, async (req, res) => {
        try {
            ...
            const duplicatedCampaign = await emailService.duplicateCampaign(id, new_name.trim(), req.user.id);
    ...
    router.get('/subscribers', authenticateToken, async (req, res) => {
        try {
            ...
            const { status, source, tags, email_verified, engagement_score_min, engagement_score_max, search, limit = 50, offset = 0 } = req.query;
            const filters = {
            ...
            if (engagement_score_min)
                filters.engagement_score_min = parseInt(engagement_score_min);
            if (engagement_score_max)
                filters.engagement_score_max = parseInt(engagement_score_max);
            if (search)
    ...
    router.post('/send-single', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
            const { to, subject, html_content, text_content, template_id, dynamic_template_data, custom_args, send_at } = req.body;
