...
exports.createSubscriptionsRouter = createSubscriptionsRouter;
...
function createSubscriptionsRouter(pool) {
    ...
    router.get('/plans', async (req, res) => {
        try {
            ...
            const plans = await subscriptionService.getPlans(creator_id);
            res.json({
                plans
            });
        }
    ...
    router.post('/plans', authenticateToken, async (req, res) => {
        try {
            ...
            const { name, description, price, currency = 'USD', features = [] } = req.body;
    ...
    router.post('/subscribe', authenticateToken, async (req, res) => {
        try {
            ...
            const { planId, paymentMethodId, trialDays, couponCode } = req.body;
            if (!planId) {
    ...
    router.get('/billing/portal', authenticateToken, async (req, res) => {
        try {
            ...
            const billingSession = await subscriptionService.createBillingPortalSession(req.user.id, return_url);
            res.json({
    ...
    router.get('/access/:planId', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
    ...
    router.get('/access-level', authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Authentication required' });
            }
